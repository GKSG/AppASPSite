...
{
    public partial class FirstTest11 : System.Web.UI.Page
    {
        private static TextBox[] Boxes = new TextBox[5];
        private static bool next = true;
        static bool start=false;
        private static KeyValuePair<int, int> index;
        private static Random randTag=  new Random((int) DateTime.Now.Ticks);
        private static Random randWord=  new Random((int) DateTime.Now.Second);
        static Random RandomTask=new Random((int) DateTime.Now.Millisecond);
        static int num = RandomTask.Next(7);
        public static Repo repo = new Repo();
        private  static ITask task = null;
        static TextBox InputWord=new TextBox ();
        static ImageButton Button=new ImageButton {ImageUrl="https://cdn3.iconfinder.com/data/icons/web-icons-21/111/validate-2-128.png" ,  OnClientClick="return click()" };
                ...
                if (start)
                {
                    if (task is MatchesTask)
                    {
                        ...
                        else
                        {
                            next = false;
                            ClientScript.RegisterStartupScript(this.GetType(), "calling",
                $"<script type=\"text/javascript\">alert(\"Wrong answer(-s), bro(\")</script>");
                        }
                    }
                        ...
                        case 0:
                            {
                                ...
                                MainPanel.Controls.Add(end);
                                MainPanel.Controls.Add(new TableCell { Controls = { Button, end } });
                        ...
                        case 3:
                            {
                                ...
                                MainPanel.Controls.Add(header); MainPanel.Controls.Add(end);
                                MainPanel.Controls.Add(sense); MainPanel.Controls.Add(end);
                                MainPanel.Controls.Add(new TableCell { Controls = { content } }); MainPanel.Controls.Add(end);
